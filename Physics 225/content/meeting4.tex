...
\subsection{Manifolds and Topological Spaces}
	...
	\subsubsection{Charts}
		...
		is a map from points from $\mcal{M}$ to $\R^n$, that's nothing but a system. $\mcal M$ is just a set, an abstract set with no coordinates, it's 
		...
		Manifolds are used here because we're giving each point in an abstract set $\mcal M$, say $x^i$, a set of coordinates $\vb{x}_i$. In other words we're giving a map from $\mcal O_1$ to 
		...
		% \]
		\[
			\psi:\mcal O \to \R^n,\quad p\mapsto(x^1(p),\ldots,x^n(p))
		\]
	...
	\subsubsection{Differentiable Manifolds}
		...
		\]
		These are called \textbf{transition functions} in the math literature. But for now, we'll call it, simply, coordinate transformations. 
